...
**Series reorder mode**:
系列详情页的短暂状态：header 左侧为退出与拖拽模式指示，成员区关闭分页并以拖拽调整 SeriesItem.order；成员封面左上角显示拖拽模式角标以标明可拖；松手即落库，本次提交的有序成员一律 `sortOrderLocked=true`，且尽量保留原先已锁定成员的 `order` 数值（仅当已锁成员相对顺序变化或夹缝插入需要时才改其数值）；退出后恢复普通系列详情（含分页）。不是 Series 上的持久字段。入口：成员卡长按，或系列详情 header 竖三点菜单中的「重新排序」。
_Avoid_: 编辑模式、排序页、拖拽模式（口语可用，领域用 Series reorder mode）；与 Library reorder mode 混称
