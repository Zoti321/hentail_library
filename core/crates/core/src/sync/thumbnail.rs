...
pub async fn generate_thumbnails(
    ...
    for comic in targets {
        ...
        match generate_and_store(db, comic).await {
            Ok(true) => {}
            Ok(false) => failed += 1,
            Err(_) => failed += 1,
        }
...
async fn generate_and_store(db: &DatabaseConnection, comic: &ComicDto) -> Result<bool, HentaiError> {
...
fn generate_thumbnail_jpeg(path: &Path, resource_type: &str) -> Result<Option<Vec<u8>>, HentaiError> {
