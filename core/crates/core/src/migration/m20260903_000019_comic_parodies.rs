...
/// Comic Parody：全局名字典 + Comic 附着 + Metadata field lock。
...
impl MigrationTrait for Migration {
  ...
  PRIMARY KEY (comic_id, parody_name),
  FOREIGN KEY(comic_id) REFERENCES comics(comic_id) ON DELETE CASCADE,
  FOREIGN KEY(parody_name) REFERENCES parodies(name) ON DELETE CASCADE
)",
            )
            .await?;
        add_bool_column(manager, "comic_meta", "parodies_locked").await?;
        Ok(())
    }
