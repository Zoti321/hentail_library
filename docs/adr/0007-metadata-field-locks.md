...
- **Comic**：`title`、`description`、`published_at`、`content_rating`、`authors`、`tags`、`languages`（整字段一把锁）。
...
- `content_rating`：扫描侧当前固定为 `unknown` 时**不覆盖**；`authors` / `tags` / `languages` 仅在扫描列表非空时替换。
