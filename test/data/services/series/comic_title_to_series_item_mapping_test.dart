...
void main() {
  const ComicTitleToSeriesItemMapping mapping = ComicTitleToSeriesItemMapping();

  test('mapComicTitleToSeriesVolume parses base and volume', () {
    final MappedSeriesVolume? actual = mapping.mapComicTitleToSeriesVolume(
      '标题A 3',
    );
  ...
  test('mapComicTitleToSeriesVolume maps 前篇 and 后篇 to volumes 1 and 2', () {
    final MappedSeriesVolume? zen = mapping.mapComicTitleToSeriesVolume(
      '标题A 前篇',
    );
    final MappedSeriesVolume? kou = mapping.mapComicTitleToSeriesVolume(
      '标题A 后篇',
    );
  ...
  test('contiguous form not used when space before trailing digits', () {
    expect(mapping.mapComicTitleToSeriesVolume('标题A 12')?.seriesName, '标题A');
  ...
  test('Comic Market tag examples map without C### in series name', () {
    ...
    expect(mapping.mapComicTitleToSeriesVolume('(C86) C2lemon@EX'), isNull);
  });
  ...
  test('parses bracket series with 卷 suffix pattern', () {
    final MappedSeriesVolume? actual = mapping.mapComicTitleToSeriesVolume(
  ...
  test('rejects bracket series with 卷00', () {
    final MappedSeriesVolume? actual = mapping.mapComicTitleToSeriesVolume(
      '[Kmoe][Fate／StayNight]卷00',
    );
    expect(actual, isNull);
  });

  test('parses 第...部 suffix with subtitle', () {
    final MappedSeriesVolume? actual = mapping.mapComicTitleToSeriesVolume(
      '騎士の誇りはなくさない 第二部 狂乱の舞',
    );
    expect(actual, isNotNull);
    expect(actual!.seriesName, '騎士の誇りはなくさない');
    expect(actual.volumeSortKey, 2);
  });

  test('parses contiguous single roman suffix after non-ascii base', () {
    final MappedSeriesVolume? actual = mapping.mapComicTitleToSeriesVolume(
      '絶対魔獣輪姦I',
    );
    expect(actual, isNotNull);
    expect(actual!.seriesName, '絶対魔獣輪姦');
    expect(actual.volumeSortKey, 1);
  });
}
