...
void main() {
  group('ResourceScanner.scanRoots', () {
    ...
    test('collects manga image directory as dir and does not recurse into it', () async {
      ...
      expect(
        result.any((e) => e.path == mangaDir.path && e.type == ResourceType.dir),
        isTrue,
      );
    });

    test('recurses non-manga directory and collects nested manga dir and files', () async {
      ...
      expect(
        result.any((e) => e.path == epubFile.path && e.type == ResourceType.epub),
        isTrue,
      );
      expect(
      ...
      expect(
        result.any((e) => e.path == cbrFile.path && e.type == ResourceType.cbr),
        isTrue,
      );
      expect(
        result.any((e) => e.path == rarFile.path && e.type == ResourceType.rar),
        isTrue,
      );
    });
  });
