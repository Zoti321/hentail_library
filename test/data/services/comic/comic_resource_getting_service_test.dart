...
void main() {
  group('ComicResourceGettingService', () {
    ...
    setUp(() async {
      tempDir = await Directory.systemTemp.createTemp('comic_resource_getting_test_');
    ...
    test('getComicContent returns sorted image files for dir', () async {
      ...
      final files = await service.getComicContent(mangaDir.path, ResourceType.dir);
    ...
    test('getComicCover prefers file named cover', () async {
      ...
      final cover = await service.getComicCover(mangaDir.path, ResourceType.dir);
    ...
    test('throws when type does not match path (file path with dir type)', () async {
    ...
    test('throws when path is empty', () async {
      expect(
        ...
        throwsA(isA<ArgumentError>().having((e) => e.message, 'message', contains('不能为空'))),
      );
    });
