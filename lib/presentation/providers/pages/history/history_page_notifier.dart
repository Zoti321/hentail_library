...
HistoryFeedViewData historyFeedView(Ref ref) {
  ...
  final List<entity.ReadingHistory> comicItems = comicsAsync.maybeWhen(
    data: (List<entity.ReadingHistory> data) => data,
    orElse: () => const <entity.ReadingHistory>[],
  );
  final List<series_entity.SeriesReadingHistory> seriesItems = seriesAsync
      .maybeWhen(
        data: (List<series_entity.SeriesReadingHistory> data) => data,
        orElse: () => const <series_entity.SeriesReadingHistory>[],
      );
  final int totalCount = comicItems.length + seriesItems.length;
  final List<HistoryGridItemDto> mergedItems = ref.watch(
    mergedHistoryGridItemsProvider,
  );
  final List<HistoryGridItemDto> continueReadingItems = mergedItems
