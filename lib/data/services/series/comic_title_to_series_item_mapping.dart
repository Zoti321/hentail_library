...
typedef MappedSeriesVolume = ({String seriesName, int volumeIndex});
...
final class ComicTitleToSeriesItemMapping {
  ...
  // --- Parsing constants (single-title rules; see file header) ---
  ...
  MappedSeriesVolume? mapComicTitleToSeriesVolume(String title) {
    ...
    return _parseSeriesNameAndVolumeOrder(withoutComiketPrefix);
  }
  ...
  static MappedSeriesVolume? _trySpacedAsciiDigitVolume(
    ...
    return (seriesName: base, volumeIndex: volumeIndex);
  }

  /// 规则 3b：空白 + 「前篇」|「后篇」→ 卷号 1 | 2。
  ...
  /// 规则 3c：基名与末尾 ASCII 数字段直接相连（数字段前一位不得为空白）。
